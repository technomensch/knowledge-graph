...
## Knowledge Graph Behaviors

When working in this project:

- **Lesson capture** — Suggest capturing lessons when bugs are solved or breakthroughs made (`kg_capture`)
- **Recall** — Search knowledge graph before answering questions about project history or past decisions (`kg_search`)
- **Session wrap** — Prompt for session summary when stopping work or approaching context limits (`kg_capture` with type `session`)
- **ADR guidance** — Suggest creating an ADR when making architecture decisions (`kg_capture` with type `adr`)
...
## Platform Preferences (Codex)

- File and content search: use standard shell tools (`find`, `grep`, `rg`)
- Avoid commands producing >50 lines of output — pipe through `head` or filter
- Use subagents for heavy file exploration to keep main context clean
