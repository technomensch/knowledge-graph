...
function registerSearchTool(server) {
    server.tool("kg_search", "Full-text search across all knowledge graph files in the active KG", {
        ...
        const dbPath = (0, fts5_js_1.getDbPath)(kgPath);
        let allResults;
        let usingFts5 = false;
        if (fs.existsSync(dbPath)) {
            try {
                allResults = (0, fts5_js_1.searchFts5)(dbPath, query, kgPath);
                usingFts5 = true;
            }
            catch (err) {
        ...
        if (format === "paths") {
            ...
            const formatted = allResults.map((r) => `[${r.matchType}${searchLabel}] ${r.relativePath}:${r.line}\n${r.context}\n`);
