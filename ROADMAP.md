...
- **ENH-064/#246 — Attribution README for initialized knowledge graphs.** Tracked on branch `v0.7.5-ENH-064-add-readme-to-graph` (stacked off `v0.7.4.1-graph-numbering-conflict-gate`). Auto-generate a README in every initialized graph pointing to KMGraph and telling a clone/fork reader without the plugin how to install it and run `/kmgraph:kmg-init`. Plan covers `kmg-init` wiring, `kmg-upgrade` backfill, and `INSTALL.md`/`docs/INSTALL.md` updates for the first-time-install-into-existing-graph path. Docs and tracking complete; implementation not started (awaiting "Execute Step 1").
