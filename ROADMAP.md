...
- **v0.6.20 — Storage migration completion (ADR-066 execution).** Fully decided, 13-task plan drafted (`~/.claude/plans/v0.6.20-storage-migration-completion.md`), not yet started (no branch, no code touched). Resolves ADR-066 (cowork/global-topic KG storage) and completes the `docs/`→`knowledge/` folder-migration sweep an independent Fable audit found incomplete (106 stale lines, 15 files, including `core/`).
- **Process/tooling gaps found 2026-07-17, all open, none yet designed or fixed:** issue-17/#175 (no recall-trigger when the assistant needs clarification mid-task), issue-18/#176 (`gov-capture-routing` referenced by 8+ commands but unreachable via the Skill tool — real file, wrong location, full provenance in the issue doc), issue-19/#177 (no hook-level enforcement for issue-creation discipline — proposes a `PostToolUse` hook, not yet designed), issue-20/#179 (this session skipped its own Bug/Enhancement Triage rule — same-feature-area check + Path F/1/2/3 routing question — for 4 filings before catching itself), ENH-048/#178 (session-wrap should verify outstanding items' status/priority is still accurate before closing out, linked to ENH-002), ENH-049/#180 (captures — does not yet design — the need to work across multiple repos/tools concurrently with different active KGs; cross-references ADR-067).
...
- **ADR-067 (proposed) — mutable `.active` switch vs context-derived KG resolution.** Raised 2026-07-15. The current model resolves "which KG" via a single mutable `.active` pointer that `kmg-switch` rewrites and every tool reads — global mutable state shared across sessions/projects, with structural cross-KG "bleed" risk (stale pointer, concurrent sessions, active≠cwd mismatch). Open question: keep the switch, or derive the KG from context (project-root/cwd, context-mode style), or hybrid — and how personal/global/cowork cross-project KGs fit. Large blast radius across all `kg_*` tools + commands; would supersede `kmg-switch` (and its issue-14/c1 fix). **Explicitly deferred beyond the current session.** Context + open decision: `knowledge/decisions/ADR-067-*.md` (proposed).
- ~~ADR-066 (proposed) — KG *content*-storage location for global-topic/cowork modes.~~ **RESOLVED 2026-07-17** (moved out of this "needs a brainstorm" list — it got one, in conversation, plus independent web research + a Fable code review). Decision: cowork mode stops being offered for new KGs (existing content detected + archived, never silently dropped); global-topic mode kept, relocates `~/.claude/knowledge-graphs/<name>/` → `~/.kmgraph/knowledge-graphs/<name>/` (no wrapper folder); `cli.ts`/MCP server becomes authoritative for the storage-mode list after its own home-option bug is fixed. Full decision recorded in `knowledge/decisions/ADR-066-*.md` (status: Accepted). Implementation planned but not started: `~/.claude/plans/v0.6.20-storage-migration-completion.md` (13 tasks — also completes the `docs/`→`knowledge/` folder-migration sweep found incomplete by an independent Fable audit, 106 stale lines across 15 files).
