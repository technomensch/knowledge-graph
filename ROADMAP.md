...
- **ADR-066 (proposed) — KG *content*-storage location for global-topic/cowork modes.** Found 2026-07-14 during the issue-14 blast-radius audit. `commands/kmg-init.md` still stores "global topic-based" and "Claude Cowork" KG *content* under `~/.claude/knowledge-graphs/` and `~/.claude/cowork-knowledge/` (lines 920-921, 1039, 1042), while `mcp-server/src/cli.ts` (the MCP init) already offers only `~/.kmgraph/` and dropped those two modes entirely — a live divergence between the two `kmg-init` implementations. Recall confirmed **no ADR** ever decided content-storage relocation (ADR-028 moved only the *personal* KG home; ADR-001 moved only `kg-config.json`; FTS5 moved only the index). This is **not** part of issue-14's config-path fix (c1/c2/c3) — it is a product decision: do global-topic/cowork storage modes still exist, and if so do they relocate to `~/.kmgraph/` per ADR-028's platform-agnostic principle, and how are the two init flows reconciled? Context + the pending decision captured in `knowledge/decisions/ADR-066-*.md` (proposed) and `knowledge/issues/issue-14/investigation-log.md` Finding 8.
