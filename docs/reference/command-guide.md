...
| `capture-corruption` | Repairs files corrupted by the filename/frontmatter-double-embed bugs (issue-46): merges cleanly-doubled frontmatter blocks, strips exact-duplicate filename prefixes. Requires `confirmBackfix: true` (see "Backfix categories" below) |
| `diff-blank-reconstruction` | Repairs session-summary files left with a permanently blank "key files modified" section by the pre-issue-47 `git diff main...HEAD` bug (issue-47): reconstructs the modified-file list from git history for cases where the recorded commit is still resolvable (via the corrected merge-base logic); an unresolvable commit (deleted branch, gc'd) gets an explanatory note instead of a fabricated list. Requires `confirmBackfix: true` (see "Backfix categories" below) |
| `plan-status-drift` | Repairs stale `~/.claude/plans/` mirrors of `knowledge/plans/*.md` plan files (issue-49). Two tiers of different reliability, never conflated: **Tier A** (mirror drift — canonical already reads `COMPLETE`, its mirror is still the frozen STOPPED placeholder, rest of the file is identical) is auto-repaired by `apply`, one direction only, canonical → mirror. **Tier B** (branch-merge inference — both copies still frozen, supporting evidence gathered from git history) is report-only: it always lands in the manual-review list, no matter how confident the evidence looks, and is never written by `apply`. Requires `confirmBackfix: true` for Tier A writes (see "Backfix categories" below); skipped automatically when there are zero Tier A findings |
| `platform-split` | Removes Claude Code–specific tool directives from `knowledge/rules.md`. Requires `confirm_platform_split: true` |
| `stale-fts5-index-format` | Rebuilds a project-local search index still at the pre-v0.7.4 name-only path (`<kg_name>.db`), which risked collisions between same-named graphs in different projects, at the new path-hashed filename (issue-55). Non-destructive — old index file is left in place |
| `stale-handoff-packages-location` | Moves pre-issue-31 handoff packages from the stray, gitignored `./handoff-packages/<date>/` into `knowledge/handoffs/<date>/` (issue-56). Deduplicates identical files; files that differ are left in place and reported for manual review |
| `connect-unregistered-graph` | Opt-in: registers an existing unregistered folder in place — no re-scaffold, no template writes — closing the dead end where `kg_config_init` refuses to scaffold over already-present, unregistered `decisions/`/`lessons-learned/` content and points here instead (issue-57). Reuses an orphaned `.kmgraph-id` marker's existing `graphId` when present (preserves continuity instead of minting a new identity); otherwise derives a unique name from the directory basename. Goes through the same `resolveRegistrationGuard` hard-block/broad-ancestor-warning check every other registration path (`kg_config_init`, the CLI's `runInit`) already uses — no exemptions. Requires no `confirmBackfix`; it registers rather than repairs content |
| `missing-root-readme` | Backfills the attribution README (ENH-064) onto graphs registered before this feature shipped — explains what KMGraph is for a reader who finds the repo without the plugin installed and how to run `/kmgraph:kmg-init` to connect. Non-destructive: never overwrites an existing `README.md`. Requires no `confirmBackfix`; purely additive |

**Backfix categories and consent** (v0.7.2): `capture-corruption`, `diff-blank-reconstruction`, `plan-status-drift`, and `platform-split` repair or remove already-written content, so all four require an explicit confirmation param in the same `apply` call (`confirmBackfix: true` for `capture-corruption`, `diff-blank-reconstruction`, and `plan-status-drift` — one shared param covering all three, not one each; `confirm_platform_split: true` for `platform-split` — independent, not an alias). Omitting the required param returns `{ error: "KMG_INPUT_REQUIRED", resolveWith: { param: "..." }, ... }` as its own content block instead of applying, so the caller can prompt and retry with the param set; any other categories in the same `apply` call still run and their results are preserved. In practice this is required-parameter enforcement, not a live back-and-forth prompt: no real interactive elicitation transport is wired up yet, so a caller that doesn't pre-set the param gets the same `KMG_INPUT_REQUIRED` response either way (after a short wait in interactive mode). `capture-corruption`, `diff-blank-reconstruction`, and `plan-status-drift` all skip asking (but still run, harmlessly, as a no-op) when a fresh re-scan finds nothing to repair — `platform-split` always asks when included in `apply`, since its "nothing found" signal is a stored schema marker rather than a live re-scan and can't safely stand in for genuine confirmation.

`plan-status-drift` is graph-independent — `knowledge/plans/` lives at the project root, not under a resolved KG's path — so it applies cleanly even when the registered KG's directory doesn't exist on disk, and inspect mode still surfaces its findings when no graph resolves at all.
