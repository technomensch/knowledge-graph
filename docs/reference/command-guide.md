...
**Backfix categories and consent** (v0.7.2): `capture-corruption`, `plan-status-drift`, and `platform-split` repair or remove already-written content, so all three require an explicit confirmation param in the same `apply` call (`confirmBackfix: true` for `capture-corruption` and `plan-status-drift` — shared param, same semantics; `confirm_platform_split: true` for `platform-split` — independent, not an alias). Omitting the required param returns `{ error: "KMG_INPUT_REQUIRED", resolveWith: { param: "..." }, ... }` as its own content block instead of applying, so the caller can prompt and retry with the param set; any other categories in the same `apply` call still run and their results are preserved. In practice this is required-parameter enforcement, not a live back-and-forth prompt: no real interactive elicitation transport is wired up yet, so a caller that doesn't pre-set the param gets the same `KMG_INPUT_REQUIRED` response either way (after a short wait in interactive mode). `capture-corruption` and `plan-status-drift` both skip asking (but still run, harmlessly, as a no-op) when a fresh re-scan finds nothing to repair — `platform-split` always asks when included in `apply`, since its "nothing found" signal is a stored schema marker rather than a live re-scan and can't safely stand in for genuine confirmation.
