...
|---|---|---|
| **lesson-capture** | "figured it out", "solved it", "pattern here is", bug breakthrough, debugging victory | `/kmgraph:kmg-capture-lesson` with pre-filled context |
| **kg-recall** | "have we done this before", "did we solve this", past decisions, history questions | `/kmgraph:kmg-recall` with extracted search terms |
| **session-wrap** | Session ending, context limit approaching, major milestone completed | `/kmgraph:kmg-session-summary` before compaction |
| **adr-guide** | "I'm thinking of using...", architecture decisions, tech choice discussions | `/kmgraph:kmg-create-adr` with decision guidance |
...
| **docs-impact-scan** | "push to origin", "push and merge", "push and merge with admin", "open PR", "create PR", "finishing up", "ready to push" | Scans for all user-facing docs affected by current branch changes; validates list with user before dispatching `/kmgraph:kmg-update-doc --user-facing` for each confirmed file |
...
| **brainstorm-recall** | `superpowers:brainstorming` invoked; fires before `adr-guide` | Invokes `kmgraph:kmg-recall` with the topic; surfaces results under "Prior Art" before any recommendation |
...
Skills provide suggestions. Commands execute actions. A skill surfaces `/kmgraph:kmg-capture-lesson` at the right moment; the command does the actual capture. Skills can be thought of as a smart prompt layer that knows when to interrupt.
