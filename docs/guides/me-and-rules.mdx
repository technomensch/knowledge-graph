...
The `me.md` and `triggers.md` files are automatically loaded into session context at each SessionStart. Workflow-phase triggers fire reliably even after context compaction; specific `rules.md` sections load on demand by anchor reference (e.g., `rules.md § Approval Gates`).
