...
graph LR
    A["📝 Capture<br/>/kmgraph:kmg-capture-lesson"] --> B["📊 Structure<br/>/kmgraph:kmg-backfill"]
    B --> C["💾 Summarize<br/>/kmgraph:kmg-session-summary"]

    accTitle: Knowledge Capture Pipeline
    accDescr: Three-step workflow: Capture lessons feeds into Structure (index into the graph), which feeds into Summarize session.
```

Run `/kmgraph:kmg-backfill` any time to index existing lessons, decisions, or chat-history into the graph, and `/kmgraph:kmg-session-summary` at the end of a session to capture what happened.
