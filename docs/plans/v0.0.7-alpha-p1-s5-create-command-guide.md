...
**Status**: ⬜ Not Started
**Parallelizable**: ✅ Yes (with Session 4)
...
- Learning path: Essential → Intermediate → Advanced
- Decision trees: "I want to..." → "Use this command"
...
**✅ CAN RUN IN PARALLEL** with Session 4 (Create CONCEPTS.md)
...
- CHEAT-SHEET.md: One-page, no explanations, just task→command mappings. Fits on a printed page.
...
- Do NOT duplicate the glossary from CHEAT-SHEET.md — link to CONCEPTS.md instead.
...
**Example output**:
```
📊 Knowledge Graph Status: my-project
...
>
> Read each command file in `/commands/` to understand its actual behavior. Do NOT assume or guess — the command files are the source of truth.

**Commands to document in this section** (in this order):

1. **`/knowledge:update-graph`** — Extracts patterns from lessons, updates KG entries, syncs MEMORY.md
2. **`/knowledge:add-category`** — Adds a custom category with optional prefix to the KG
3. **`/knowledge:session-summary`** — Creates a summary of the current or recent work session
4. **`/knowledge:list`** — Lists all configured knowledge graphs from kg-config.json
5. **`/knowledge:switch`** — Changes the active knowledge graph
6. **`/knowledge:check-sensitive`** — Scans KG files for potentially sensitive information
7. **`/knowledge:config-sanitization`** — Interactive wizard to set up pre-commit sanitization hooks
8. **`/knowledge:extract-chat`** — Extracts chat history from Claude/Gemini local logs
...
1. **`/knowledge:meta-issue`** — Initialize structured tracking for complex multi-attempt problems
2. **`/knowledge:start-issue-tracking`** — Initialize issue tracking with documentation and Git branch
3. **`/knowledge:update-issue-plan`** — Synchronize KG extraction with active plans and issue tracking
4. **`/knowledge:link-issue`** — Manually link existing lesson or ADR to a GitHub issue
5. **`/knowledge:archive-memory`** — Archive stale MEMORY.md entries to prevent bloat
6. **`/knowledge:restore-memory`** — Restore archived entries from MEMORY-archive.md
7. **`/knowledge:sync-all`** — Run the full 4-step sync pipeline in one command
...
3. **`/knowledge:sync-all`**
   - Full 4-step pipeline (capture → update → sync → archive)
   - Comprehensive sync
   - Use: Weekly deep sync or before sharing

**When to use which**:
- Just solved problem → `capture-lesson`
...
- [ ] Difficulty badges consistent (🟢🟡🔴)
- [ ] Sections logically ordered (Essential → Intermediate → Advanced)
...
- Created progressive learning path (Week 1 → Month 2+)
...
- [ ] Merge v0.0.7-alpha-p1-s5 back to base
...
**Status**: ⬜ Not Started
**Last Updated**: 2026-02-20 (Opus review revision — expanded Intermediate/Advanced command guidance)
