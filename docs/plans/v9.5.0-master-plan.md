# Knowledge Graph Plugin for Claude Code — Master Plan
...
**Current location:** `optimize-my-resume/docs/plans/v9.5.0-*` (temporary — tied to source project)
...
**Why this change:** The knowledge graph infrastructure built across v9.3.x-v9.4.0 (lessons-learned, KG entries, MEMORY.md sync, meta-issue tracking, chat extraction, session summaries) is domain-agnostic. Packaging it as a Claude Code plugin makes it reusable across any project — not just resume optimization.
...
1. **Claude Code users get full automation**: Install plugin → Run `/knowledge:init` → `/knowledge:capture-lesson` → Everything scaffolded automatically
2. **Local LLM users get the core system**: Clone repo → Copy `core/` → Follow manual workflows in `core/docs/WORKFLOWS.md`
3. **Other IDE users can adapt**: Fork repo → Keep `core/` → Replace Claude skills with Cursor rules / Continue commands / Aider scripts
...
**Target Audiences:**
- **Primary (v1.0)**: Claude Code users → Get everything via plugin
- **Secondary (documented)**: Users of local LLMs, Cursor, Continue.dev, Aider → Use `core/` manually
- **Tertiary (future)**: Platform adapter developers → Fork and create integrations for their tools

---

## Design Principles: Framework vs. Content

### 1. **Clear Separation: Framework ≠ Content**
...
- **Using Claude Code?** → Install the plugin normally: `/plugin install knowledge`. Get automated skills like `/knowledge:capture-lesson`.
- **Using a local LLM or different IDE?** → Clone the repo, copy `core/` to your project, manually follow workflows documented in `core/docs/WORKFLOWS.md`.
- **Want to adapt for another platform (Continue.dev, Cursor, Aider)?** → Fork the repo, keep `core/`, replace Claude Code skills/hooks with your platform's automation primitives.
...
```
knowledge-graph-plugin/                # ← Claude Code plugin (install root)
│
├── .claude-plugin/
│   └── plugin.json                    # Plugin manifest (Claude Code discovers this at root)
│
├── skills/                            # 🔌 CLAUDE CODE AUTOMATION LAYER
│   ├── init/
│   │   └── SKILL.md                   # /knowledge:init (wizard-based setup)
│   ├── list/
│   │   └── SKILL.md                   # /knowledge:list (show all configured KGs)
│   ├── switch/
│   │   └── SKILL.md                   # /knowledge:switch (change active KG)
│   ├── add-category/
│   │   └── SKILL.md                   # /knowledge:add-category (extend KG structure)
│   ├── configure-sanitization/
│   │   └── SKILL.md                   # /knowledge:configure-sanitization (user pre-commit setup)
│   ├── check-sensitive/
│   │   └── SKILL.md                   # /knowledge:check-sensitive (scan for sensitive data)
│   ├── link-issue/
│   │   └── SKILL.md                   # /knowledge:link-issue (link lesson to GitHub issue)
│   ├── capture-lesson/
│   │   └── SKILL.md                   # /knowledge:capture-lesson (with git metadata)
│   ├── recall/
│   │   └── SKILL.md                   # /knowledge:recall
│   ├── update-graph/
│   │   └── SKILL.md                   # /knowledge:update-graph
│   ├── sync-all/
│   │   └── SKILL.md                   # /knowledge:sync-all
│   ├── update-issue-plan/
│   │   └── SKILL.md                   # /knowledge:update-issue-plan (with PR integration)
│   ├── session-summary/
│   │   └── SKILL.md                   # /knowledge:session-summary
│   ├── extract-chat/
│   │   └── SKILL.md                   # /knowledge:extract-chat
│   ├── meta-issue/
│   │   └── SKILL.md                   # /knowledge:meta-issue
│   └── status/
│       └── SKILL.md                   # /knowledge:status (active KG info + quick reference)
│
├── agents/
│   └── knowledge-reviewer.md          # Subagent for KG entry quality review
│
├── hooks/
│   └── hooks.json                     # SessionStart: memory check, PostToolUse: lesson suggestions
│
├── config/
│   └── kg-config-template.json        # Template for ~/.claude/kg-config.json (tracks multiple KGs)
│
├── scripts/
│   ├── check-memory.sh                # Hook script for SessionStart (Claude-specific)
│   ├── prepare-mcp.sh                 # MCP server build helper (checks Node.js, installs deps, builds)
│   └── validate-plugin.sh             # Pre-publish sanitization check (scans for project-specific terms)
│
├── docs/
│   └── SETUP.md                       # Post-install instructions (Claude Code specific)
│
├── core/                              # ⭐ PLATFORM-AGNOSTIC CORE
│   ├── README.md                      # How to use without Claude Code (manual workflows)
│   │
│   ├── templates/
│   │   ├── knowledge/
│   │   │   ├── patterns.md            # KG patterns template (empty + instructions)
│   │   │   ├── gotchas.md             # KG gotchas template (empty + instructions)
│   │   │   ├── concepts.md            # KG concepts template (empty + instructions)
│   │   │   ├── architecture.md        # KG architecture template (empty + instructions)
│   │   │   ├── workflows.md           # KG workflows template (empty + instructions)
│   │   │   ├── index.md               # Master navigation hub template
│   │   │   └── entry-template.md      # Template for new KG entries
│   │   ├── lessons-learned/
...
│   │   │   ├── description.md         # Living document template
│   │   │   ├── implementation-log.md  # Attempt timeline template
│   │   │   ├── test-cases.md          # Validation criteria template
│   │   │   └── attempt-template/
│   │   │       ├── solution-approach.md
│   │   │       └── attempt-results.md
│   │   ├── sessions/
│   │   │   └── session-template.md    # Session summary template
│   │   └── MEMORY-template.md         # Starter MEMORY.md for new projects
│   │
│   ├── examples/
│   │   ├── lessons-learned/
│   │   │   ├── architecture/
│   │   │   │   ├── Example_Three_Tier_Sync.md           # Generalized from Shadow Sync
│   │   │   │   └── Example_Claude_Code_Skills_Arch.md   # Skill discovery patterns
│   │   │   ├── process/
│   │   │   │   ├── Example_LLM_Constraint_Patterns.md   # Generalized from enforcement
│   │   │   │   ├── Example_SessionStart_Automation.md   # Hook-based automation
│   │   │   │   ├── Example_Identifier_Decoupling.md     # Dual-ID policy
│   │   │   │   └── Example_Agentic_Momentum.md          # Metric-Only Reporting
│   │   │   ├── patterns/
│   │   │   │   └── Example_Memory_System_Foundation.md  # Four-Pillar origin story
│   │   │   └── debugging/
│   │   │       └── Example_Multi_Platform_Debugging.md  # Generalized
│   │   ├── knowledge/
│   │   │   ├── sample-patterns.md     # 12 reusable patterns (see Section below)
│   │   │   ├── sample-concepts.md     # 6 reusable concepts (see Section below)
│   │   │   └── sample-gotchas.md      # 6 reusable gotchas (see Section below)
│   │   ├── decisions/
│   │   │   ├── ADR-001-example.md     # Example ADR (dual-format documentation)
│   │   │   └── ADR-002-example.md     # Example ADR (bidirectional KG-memory sync)
│   │   └── meta-issue/
│   │       └── example-performance-saga/  # Generalized from enforcement saga
│   │           ├── README.md
│   │           ├── description.md
│   │           └── attempts/
│   │               ├── 001-baseline/
...
│   │
│   ├── scripts/
│   │   ├── extract_claude.py          # Claude JSONL extraction (platform-agnostic)
│   │   ├── extract_gemini.py          # Gemini JSON/Protobuf extraction
│   │   ├── chat_extractor_base.py     # Shared utilities
│   │   ├── run_extraction.py          # Master controller
│   │   └── requirements.txt           # Python dependencies (stdlib only)
│   │
│   ├── examples-hooks/
│   │   └── pre-commit-sanitization.sh # Optional pre-commit hook for sensitive data checks
│   │
│   └── docs/
│       ├── ARCHITECTURE.md            # How the KG system works (conceptual)
│       ├── PATTERNS-GUIDE.md          # Writing good KG entries + abstraction best practices
│       ├── WORKFLOWS.md               # Manual workflows (for non-Claude users)
│       ├── META-ISSUE-GUIDE.md        # When/how to use meta-issue tracking
│       ├── SANITIZATION-CHECKLIST.md  # Privacy checklist before public sharing
│       └── PLATFORM-ADAPTATION.md     # How to adapt core for Continue/Cursor/Aider/local LLMs
│
├── mcp-server/                        # 🔌 MCP DATA LAYER (Phase 4)
│   ├── src/
│   │   ├── index.ts                   # MCP server entry point
│   │   ├── tools/
│   │   │   ├── config.ts              # kg_config_init, kg_config_list, kg_config_switch, kg_config_add_category
│   │   │   ├── search.ts             # kg_search (full-text across KG files)
│   │   │   ├── scaffold.ts           # kg_scaffold (create from templates)
│   │   │   └── sanitization.ts       # kg_check_sensitive
│   │   └── resources/
│   │       ├── config.ts              # kg://config resource
│   │       └── templates.ts           # kg://templates/{name} resource
│   ├── package.json
│   └── tsconfig.json
│
├── README.md                          # Plugin overview, installation, portability info
├── LICENSE                            # MIT
└── CHANGELOG.md
```
...
- Non-Claude users: `git clone` → copy `core/` → follow `core/docs/WORKFLOWS.md`
...
### 5. `/knowledge:configure-sanitization` ⭐ NEW
**Source:** NEW skill
...
### 6. `/knowledge:check-sensitive` ⭐ NEW
**Source:** NEW skill
...
- docs/lessons-learned/process/debugging-auth.md:42 — email: john@company.com
- docs/knowledge/patterns.md:15 — URL: https://api.internal.company.com
Review these entries before pushing to public repository.
```

### 7. `/knowledge:link-issue` ⭐ NEW
**Source:** NEW skill
...
```markdown
📚 Lesson Learned: [Title](link-to-file)
...
### 8. `/knowledge:capture-lesson` ⚠️ ENHANCED
...
3. Prompt for issue number or auto-detect from branch name (e.g., `fix/issue-41-auth` → issue #41)
4. Insert metadata in YAML frontmatter
5. If issue specified + GitHub enabled → Post comment to issue with lesson link
...
### 10. `/knowledge:update-graph` ⚠️ ENHANCED
...
- Keep 7-step process (identify → extract → verify → create/update → cross-ref → quality check → MEMORY.md sync)
...
### 12. `/knowledge:update-issue-plan` ⚠️ ENHANCED
...
#### 1. **Lesson → GitHub Issue**
When lesson is created/committed:
- If issue number in metadata → Post comment to GitHub issue:
  ```
  📚 Lesson Learned: [Lesson Title](link-to-file)
...
1. **First-run detection:** Check if `~/.claude/kg-config.json` exists. If not → output: "No knowledge graph configured. Run `/knowledge:init` to get started."
2. **MEMORY.md check:** If config exists, check if MEMORY.md exists and has been updated recently. If stale (>7 days since last KG sync) → add context reminder.
3. **Active KG validation:** If config references a path that no longer exists → warn user.

**Note:** The commit/branch validation hooks from v9.4.0 are NOT included in the plugin — those are project-specific governance. The plugin focuses on knowledge capture, not git enforcement.
...
## Example Knowledge Graph Entries (Full Library — ~30 examples)

All example files include clear `<!-- THIS IS AN EXAMPLE — Replace with your project's patterns -->` headers.

### Sample Patterns (12 entries in `examples/knowledge/sample-patterns.md`)

| # | Pattern Name | Source | Generalization |
|---|-------------|--------|---------------|
| 1 | Multi-Tier Synchronization | Shadow Sync Protocol | High — replace project-specific tier names with generic |
| 2 | Bidirectional Memory Sync | ADR-011 | None — already domain-agnostic |
| 3 | Smart Defaults | v9.4.0 skill restructure | None — already domain-agnostic |
| 4 | Automated Orchestrator | know-sync-all | Low — replace KG-specific examples with generic pipeline |
| 5 | Meta-Issue Tracking | ADR-008 | Medium — replace enforcement saga with performance example |
| 6 | Four-Pillar Memory | patterns.md entry | None — THE core architecture of the plugin |
| 7 | Project Memory System | patterns.md entry | Low — remove project-specific paths |
| 8 | Category Auto-Detection | patterns.md entry | None — already generic keyword→category mapping |
| 9 | Template-Driven Docs | patterns.md entry | None — already generic document creation pattern |
| 10 | Identifier Decoupling | patterns.md entry | None — local vs platform ID mapping applies to any project |
| 11 | Positive Constraint Framing | patterns.md entry | None — universal LLM prompt engineering pattern |
| 12 | Functional Directory Structure | patterns.md entry | None — generic directory naming pattern |

### Sample Concepts (6 entries in `examples/knowledge/sample-concepts.md`)

| # | Concept Name | Source | Generalization |
|---|-------------|--------|---------------|
| 1 | Memory Systems (Four-Pillar) | concepts.md | None — core plugin concept |
| 2 | Cross-Referencing | concepts.md | None — bidirectional linking system |
| 3 | Template-Driven Consistency | concepts.md | None — standardization approach |
| 4 | Categorization | concepts.md | None — knowledge organization strategy |
| 5 | Automation Strategy | concepts.md | Low — rename skill references to plugin namespace |
| 6 | Passive vs Active Enforcement | concepts.md | Medium — generalize guardrail examples |

### Sample Gotchas (6 entries in `examples/knowledge/sample-gotchas.md`)

| # | Gotcha Name | Source | Generalization |
|---|------------|--------|---------------|
| 1 | Agent Governance Drift | gotchas.md | None — universal: AI skips documentation steps |
| 2 | Plan File Location | gotchas.md | None — temp vs permanent file gotcha |
| 3 | Flat Skill Discovery Constraint | gotchas.md | None — critical Claude Code plugin knowledge |
| 4 | HEREDOC Commit Message Parsing | gotchas.md | None — useful for anyone writing hooks |
| 5 | The Vibe-Coding Drift | gotchas.md | Medium — generalize resume refs to "project content" |
| 6 | Instructional Saturation | gotchas.md | Medium — generalize examples to generic long-context |

### Sample Lessons Learned (7 files in `examples/lessons-learned/`)

| # | Lesson File | Source | Generalization |
|---|------------|--------|---------------|
| 1 | `architecture/Example_Three_Tier_Sync.md` | Shadow Sync Protocol lesson | High — replace project tiers with generic config tiers |
| 2 | `architecture/Example_Claude_Code_Skills_Arch.md` | Skills Architecture lesson | None — understanding skill discovery is universal |
| 3 | `process/Example_LLM_Constraint_Patterns.md` | Effective LLM Constraints | Low — already domain-agnostic |
| 4 | `process/Example_SessionStart_Automation.md` | SessionStart Automation | Low — rename skills to plugin namespace |
| 5 | `process/Example_Identifier_Decoupling.md` | Identifier Decoupling | None — Dual-ID policy is universal |
| 6 | `process/Example_Agentic_Momentum.md` | Agentic Momentum Governance | Medium — generalize resume refs to "project output" |
| 7 | `patterns/Example_Memory_System_Foundation.md` | Memory System Phase 1 | Low — THE origin story, already mostly generic |

### Sample ADRs (2 files in `examples/decisions/`)

| # | ADR File | Source | Generalization |
|---|---------|--------|---------------|
| 1 | `ADR-001-example.md` | ADR-001 Dual-Format Documentation | None — `.txt` for LLMs + `.md` for humans is universal |
| 2 | `ADR-002-example.md` | ADR-011 KG ↔ Memory Sync | None — bidirectional sync already domain-agnostic |

### Sample Meta-Issue (1 directory in `examples/meta-issue/`)

| # | Files | Source | Generalization |
|---|-------|--------|---------------|
| 1 | `example-performance-saga/` (README, description, 2 attempts) | v9.3.x enforcement saga | High — replace enforcement with generic performance investigation |

### Generalization Guidelines for All Examples

For each example file:
1. Add `<!-- THIS IS AN EXAMPLE -->` header with instructions to replace
2. Replace project-specific terms: "resume" → "project output", "enforcement" → "compliance", "optimization-tools/" → "config-modules/"
3. Keep the structural patterns (Problem → Root Cause → Solution → Replication → Lessons)
...
│   └── marketplace.json
├── plugins/
│   └── knowledge/               # The plugin lives here
│       ├── .claude-plugin/
│       │   └── plugin.json
│       ├── skills/
│       └── ...
...
**Why:**
- ✅ Copy existing working files (massive token savings)
- ✅ Test in real environment before extraction
- ✅ Catch integration issues early
- ✅ Preserve working code (no rewriting from scratch)

**Location during development:** `/Users/mkaplan/Documents/GitHub/optimize-my-resume/knowledge-graph-plugin/`

---

## Implementation Phases

All 16 skills, MCP server, and ~72+ files ship as v1.0. The build is split into **5 phases across 4 calendar sessions** — Phases 2, 3, and 4 can run in parallel.

**Location during all phases:** `/Users/mkaplan/Documents/GitHub/optimize-my-resume/knowledge-graph-plugin/`
**Gitignore:** Add `knowledge-graph-plugin/` to project `.gitignore` during development.

### Dependency Graph & Parallelization

```
Phase 1: Foundation (Session 1 — Claude Code)
   ↓ must complete first
   ├── Phase 2: New Skills        (Session 2 — Claude Code)     ──┐
   ├── Phase 3: Examples + Docs   (Session 2 — Opus/Antigravity) ──┤ PARALLEL
   ├── Phase 4: MCP Server        (Session 2 or 3 — Claude Code) ──┘
   ↓ all above must complete
Phase 5: Sanitization + Testing + Publishing (Session 3 or 4 — Claude Code)
```
...
- `config/kg-config-template.json` — Multi-KG configuration schema
- `LICENSE` — MIT license
- `CHANGELOG.md` — Start at v1.0.0
- `ROADMAP.md` — Future enhancements and known considerations (see content below)
...
- Current search is full-text file walk — works for <500 files
...
- v1.0 → v1.1 config changes need graceful migration
...
- `kg_git_metadata` — Capture branch, commit, author, PR, issue
- `kg_link_issue` — Update YAML frontmatter + post GitHub comment
- `kg_extract_chat` — Run Python extraction scripts
```
...
|-------|------|-----------|
| `/knowledge:init` | `skills/init/SKILL.md` | High — wizard with 5 prompts, creates dirs + config |
| `/knowledge:list` | `skills/list/SKILL.md` | Low — read config, format output |
| `/knowledge:switch` | `skills/switch/SKILL.md` | Low — update active field in config |
| `/knowledge:add-category` | `skills/add-category/SKILL.md` | Medium — create dirs, update config |
| `/knowledge:configure-sanitization` | `skills/configure-sanitization/SKILL.md` | Medium — wizard, install hook |
| `/knowledge:check-sensitive` | `skills/check-sensitive/SKILL.md` | Medium — scan files with regex patterns |
| `/knowledge:link-issue` | `skills/link-issue/SKILL.md` | Medium — update frontmatter + GitHub comment |
| `/knowledge:status` | `skills/status/SKILL.md` | Low — read config + count files + show quick reference |

#### 2.2: Smoke test new skills
Quick verification that each skill loads and basic flow works:
- [ ] `/knowledge:init` — wizard prompts appear, creates dirs
- [ ] `/knowledge:list` — displays configured KGs
- [ ] `/knowledge:switch` — changes active KG
- [ ] `/knowledge:add-category` — adds category
- [ ] `/knowledge:configure-sanitization` — wizard runs
- [ ] `/knowledge:check-sensitive` — scans and reports
- [ ] `/knowledge:link-issue` — updates frontmatter
...
### Phase 3: Examples + Documentation (Session 2 — Opus in Antigravity)
...
**Why Antigravity:** This is pure markdown writing — read existing files, generalize content, write new docs. No file system operations or code execution needed. Provide the Antigravity agent with this plan + the source files to generalize.
...
|--------|--------|---------------------|
| `docs/knowledge/patterns.md` (12 entries) | `core/examples/knowledge/sample-patterns.md` | Medium — replace project refs |
...
2. Find/replace: `resume optimization` → `output optimization`, `enforcement` → `compliance`, `v9.3.x` → `v2.x`
3. Replace specific names with roles: "John Smith" → "Backend Developer"
4. Replace project-specific skill names: `/gov-git-branch` → `/governance:validate-branch`
5. Verify no broken internal cross-references

#### 3.2: Write all documentation (10 files)

**Claude Code Documentation:**
1. `docs/SETUP.md` — Post-install instructions (replaces setup-project.sh entirely)
...
2. Root `README.md` — Overview, installation, quick start, portability note, MCP server setup
...
3. `core/README.md` — How to use without Claude Code (manual workflows + MCP server option)
4. `core/docs/ARCHITECTURE.md` — How the KG system works (conceptual)
5. `core/docs/PATTERNS-GUIDE.md` — Writing good KG entries + abstraction best practices + privacy
...
8. `core/docs/PLATFORM-ADAPTATION.md` — Adapt for Continue.dev, Cursor, Aider, local LLMs (include MCP setup)
9. `core/docs/SANITIZATION-CHECKLIST.md` — Privacy checklist before public sharing
...
**Recommendation: TypeScript** — most common MCP language, better type safety, wider MCP ecosystem support. If Python preferred (reuses extraction scripts), use Python MCP SDK instead.
...
### Phase 5: Sanitization, Testing & Publishing (Final Session — Claude Code)
...
**Integration tests:**
- [ ] Init → capture-lesson → update-graph → recall (full workflow)
- [ ] Init → switch → capture-lesson (multi-KG workflow)
...
- Phases 2 + 3 run simultaneously → **~1 calendar session saved**
- Phase 4 can overlap with Phase 2 in same Claude Code session → potential **additional session saved**
- Worst case: 4 sessions sequential. Best case: 3 sessions (Phase 2+4 in one, Phase 3 parallel)

**Quality Benefits:**
- ✅ Preserve battle-tested code (no rewrite bugs)
- ✅ Consistent with existing patterns
- ✅ Test in real environment before extraction
- ✅ Iterative refinement (fix issues locally before publishing)

**Workflow:**
1. Build in `optimize-my-resume/knowledge-graph-plugin/`
2. Test with `claude --plugin-dir ./knowledge-graph-plugin`
...
### 3. MEMORY.md Location Discovery ⚠️ OPEN
...
3. If found → use that path. If not found → prompt user to provide or create new.
...
The shell scripts (check-memory.sh, prepare-mcp.sh) need to work on macOS and Linux. Avoid macOS-specific `sed -i ''` syntax — use portable alternatives.
...
### 8. Template Customization ⚠️ OPEN
...
Before publishing, create a test project that installs the plugin fresh and runs through the full workflow: install → setup → capture lesson → update KG → recall → sync-all.
...
- Manual workflow: Create lesson → Extract to KG → Link to MEMORY.md
...
1. **Documentation warning** (in SETUP.md, README.md, PATTERNS-GUIDE.md):
   - "⚠️ WARNING: Never commit personal information, credentials, API keys, internal system details, or confidential project data to your knowledge graph."
...
- README.md: "This plugin provides a framework for knowledge management. The examples are illustrative — customize them for your project."
...
- `resume optimization` → `output quality optimization` or `document optimization`
- `enforcement saga` → `compliance saga` or `quality assurance saga`
- `Shadow Sync Protocol` → `Multi-Tier Synchronization Pattern` (when used as example)
- `v9.3.x` → `v2.x` or `phase 2`
- `PROJECT-INSTRUCTIONS.md` → `PROJECT-CONFIG.md` (generic)
- Specific skill names (`/gov-git-branch`) → generic (`/governance:validate-branch`) in examples
...
| Scripts — Root | 2 | check-memory.sh, validate-plugin.sh | 1 + 5 |
| Scripts — Core | 5 | 5 Python chat extractors (in core/scripts/) | 1 |
| Config | 1 | config/kg-config-template.json | 1 |
| Templates | 14 | KG (7), lessons (2), decisions (2), meta-issue (5+), sessions (1), MEMORY (1) | 1 |
| Examples — KG | 3 | sample-patterns.md (12), sample-concepts.md (6), sample-gotchas.md (6) | 3 |
| Examples — Lessons | 7 | architecture/ (2), process/ (4), patterns/ (1) | 3 |
| Examples — ADRs | 2 | ADR-001-example.md, ADR-002-example.md | 3 |
| Examples — Meta-Issue | 4+ | example-performance-saga/ (README, description, 2 attempts) | 3 |
| Examples — Hooks | 1 | core/examples-hooks/pre-commit-sanitization.sh | 3 |
| Documentation — Claude | 2 | docs/SETUP.md, root README.md | 3 |
| Documentation — Core | 7 | core/README.md + core/docs/{ARCHITECTURE, PATTERNS-GUIDE, WORKFLOWS, META-ISSUE-GUIDE, PLATFORM-ADAPTATION, SANITIZATION-CHECKLIST}.md | 3 |
...
| `examples/decisions/ADR-002-example.md` | ADR-011 KG ↔ Memory Sync | None (already domain-agnostic) |
