---
id: capture-lessons-learned
title: Capture Lessons Learned
sidebar_label: Capture Lessons Learned
description: How to turn a bug fix or breakthrough into a durable, searchable KG entry.
---

# Capture Lessons Learned

> "I solved something. How do I save it?"

This is the most common KMGraph action. When you fix a bug, reach a breakthrough, or learn something that will save you time later, you capture it — so your future self (and your AI assistant) can find it.

## The command

```
/kmgraph:capture-lesson
```

Run this at the end of any session where something clicked. Claude will ask you a few questions and write a structured entry to your knowledge graph.

## What gets captured

A lesson entry records:
- **What happened** — the problem or situation
- **What you learned** — the insight or fix
- **Why it matters** — context that makes it findable later

## When to capture

Capture when:
- You fixed a bug that took more than 30 minutes
- You made a decision you might revisit
- You found a pattern worth reusing
- You hit a gotcha that future-you will hit again

Don't wait until the end of the week — capture while the context is fresh.

## What's next

Not sure what type of entry to create? See [What to Capture](./what-to-capture.md) for a guide on choosing between lessons, decisions, patterns, and meta-issues.

## Related

- [What to Capture](./what-to-capture.md)
- [Capture from a Bugfix](./capture-from-bugfix.md)
- [Architecture Decisions](./architecture-decisions.md)
