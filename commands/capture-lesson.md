...
**Create New Document:**
- `/kmgraph:capture-lesson` → Create new document (Steps 1-5 below)
- `/kmgraph:capture-lesson <topic>` → Create new document with topic

**Update Existing Document:**
- `/kmgraph:capture-lesson update <filename>` → Update existing document (Step 0 below)
- Example: `/kmgraph:capture-lesson update Lessons_Learned_Chat_History_Workflow.md`

---

## Step 0: Update Existing Document

**When user types:** `/kmgraph:capture-lesson update <filename>`
...
**Example:**
```
User: "/kmgraph:capture-lesson API error handling"
...
/kmgraph:recall "API error handling"
...
1. ✅ Yes, create linked ADR — Launch /kmgraph:create-adr with pre-filled context
2. Lesson only — Skip ADR creation (lesson is complete as-is)
3. Skip — Create ADR later via /kmgraph:create-adr
```
...
Launching /kmgraph:create-adr...
```

#### Step 4.8.2: Launch ADR Creation Flow

Launch the full `/kmgraph:create-adr` flow inline:
...
**User types:** `/kmgraph:capture-lesson`
...
- `/kmgraph:link-issue` - Manually link existing lesson to GitHub issue
- `/kmgraph:create-adr` - Create a standalone ADR (without lesson capture)

---

**Created:** 2026-02-12
**Version:** 1.5
**Usage:** Type `/kmgraph:capture-lesson` when you want to document a problem solved or pattern implemented
