---
description: Create or register a global personal knowledge graph for cross-project lessons
allowed-tools: Bash, Read, Write, kg_fts5_rebuild
---

# /kmgraph:init-global-kg

Create a global personal knowledge graph at `~/.claude/knowledge-graph/` for capturing
...
- After `/kmgraph:init` (skipped the global KG offer)
- When you want a dedicated place for workflow lessons, cross-project gotchas, and personal ADRs
- When `/kmgraph:capture-lesson` asks "Save to project KG or personal KG?" but no global KG exists yet
...
2. Registers it in `~/.claude/kg-config.json` as `type: "global"` with name `"personal"`
...
`/kmgraph:recall` searches both project and global KGs automatically.

---

## Steps

### Step 1: Check for existing global KG

Read `~/.claude/kg-config.json`. Look for any entry with `type: "global"`.
...
3. Register a different path as global KG
4. Cancel
```
...
Store resolved path as `{global_kg_path}`.
...
```bash
mkdir -p "{global_kg_path}"/{knowledge,lessons-learned,decisions,sessions}
mkdir -p "{global_kg_path}/lessons-learned"/{architecture,debugging,patterns,process}
```
...
for f in patterns.md gotchas.md concepts.md architecture.md workflows.md index.md; do
  ...
  dest="{global_kg_path}/knowledge/$f"
...
Call `kg_fts5_rebuild` with `kgPath: "{global_kg_path}"`.
...
What changed:
  • Registered as "personal" (type: global) in ~/.claude/kg-config.json
...
- `/kmgraph:init` — Full KG initialization wizard (includes global KG offer)
- `/kmgraph:capture-lesson` — Capture lessons; shows KG picker when multiple KGs registered
- `/kmgraph:recall` — Search across project and global KGs
