...
- **Every graph-registration path now shares one `resolveRegistrationGuard`** — the new connect category initially registered a folder without the shared home/root hard-block and broad-ancestor-warning check that `kg_config_init` and the CLI's `runInit` already ran, so a monorepo root with an unrelated top-level `decisions/` folder could have been silently registered as a brand-new knowledge graph. Closed: MCP-native init, CLI init, and the connect category now all call the identical guard. Closes issue-57.
- **`scaffoldGraphDirectory`'s template routing had drifted from the wizard's canonical routing** — it was copying starter templates into live content directories instead of a dedicated `templates/` directory, was missing `entry-template.md` entirely, and was sourcing the wrong (longer) variants of `me.md`/`rules.md`/`triggers.md`. Rewritten to match the wizard exactly, and a scaffold-then-refuse file leak — where scaffold files could be written to disk before a refusal check aborted registration — is closed on both the MCP-native and CLI init paths. Closes issue-58.
