...
**`/knowledge:capture-lesson [title]`**
...
**`/knowledge:meta-issue [issue-number]`**
...
**`/knowledge:configure-sanitization`**
...
```
1. /knowledge:capture-lesson debugging-session-title
...
```
1. /knowledge:recall "problem description"
...
- Use `/knowledge:link-issue` for bidirectional references
- Use `/knowledge:meta-issue` for complex recurring issues
...
- Use `/knowledge:recall` to find relevant lessons
...
- Use `/knowledge:status` for quick reference and current KG info
...
- Run `/knowledge:check-sensitive` to scan for sensitive data
- Configure sanitization hooks with `/knowledge:configure-sanitization`
