...
**Protocol Steps:**
1. **State Initialization** — Output STRICT EXECUTION MODE banner before any action. Then, once the user gives explicit "YES" to proceed with the plan's Step 1, edit the plan file's own Safety Header STATUS line to read `🟡 IN PROGRESS`, replacing whatever it currently says — do not assume it reads `🔴 AWAITING APPROVAL` (a plan approved before this instruction landed, a resumed session, or an older Safety Header variant such as `🔴 STOPPED (Waiting for Manual Approval of Step 0)` may read something else entirely). This is a real write to the plan file the skill is executing from, not a chat-only status report.
...
7. **Completion Verification** — Quote each success criterion and verify it. Edit the plan file's own Safety Header STATUS line to read `✅ COMPLETE`, replacing whatever it currently says (a real write to the plan file, not a chat-only status report; this line is protocol-owned metadata and is exempt from the Data Integrity Audit). Output completion status. Then, as part of that completion output, explicitly answer: "Does this change alter how KG content is captured, stored, or structured? If yes: does `kg_upgrade` need a new/updated category for it, and is that category reachable through `/kmgraph:kmg-init`'s wizard, not just via a raw `kg_upgrade apply` call?"
