...
- [x] `commands/kmg-extract-chat.md`'s Gemini format description updated to reflect the `.jsonl` streaming path once shipped. Verified on `main` (v0.6.16): `commands/kmg-extract-chat.md:6` reads `Gemini (.json/.jsonl/.pb)` and `docs/reference/command-guide.md:779` documents `session-*.json` [pre-0.42.0] and `session-*.jsonl` [0.42.0+, streaming format]. Grep of `docs/`, `commands/`, `skills/`, `core/` found no remaining Gemini-format wording claiming a single `.json`-only path.
