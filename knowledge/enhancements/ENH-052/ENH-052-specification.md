...
- **[issue-49](../../issues/issue-49/issue-49-description.md)** — every plan
  generated by `kmg-start-issue-tracking.md` is born with a hardcoded Safety
  Header (`STATUS: STOPPED, waiting for Manual Approval of Step 1`) that nothing
  downstream ever rewrites; five fully-merged `v0.7.1.x` plans all still read
  "waiting for Step 1" months after completion. Backlinked 2026-08-17.
- **[issue-50](../../issues/issue-50/issue-50-description.md)** — the general
  form of the pattern, named explicitly: nothing checks whether a bug fix that
  touches captured KG content needs a `kg_upgrade` backfix category, or that
  the category (once added) is actually wired into the user-facing upgrade
  path. issue-46/47/48 are its case studies, not separate instances.
  Backlinked 2026-08-17.
- **[issue-51](../../issues/issue-51/issue-51-description.md)** — a live
  instance of issue-50's gap, found independently of any 46/47/48 work:
  `kmg-upgrade-inspector.md`'s hardcoded category allow-list has drifted from
  `upgrade.ts`'s real 9-category schema; `capture-corruption` and
