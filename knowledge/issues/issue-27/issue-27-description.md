...
- **Not yet resolved, same family, lower priority:** `kmg-init-personal-kg.md:346-347` (wiki-link content write-back) — writes arbitrary markdown, no destination-content check either, but no `jq empty`-equivalent validity check applies cleanly since the content isn't JSON. Flagged by the same audit, deferred pending a real design decision on what "valid" means for free-form content — distinct from this issue's fix, which is a direct destination-existence check, not a validity-format problem.
