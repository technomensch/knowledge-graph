...
**Created:** 2026-07-08
**Status:** ✅ All 6 post-merge regression findings fixed (v0.6.18, this branch) — implemented, unit- and behaviorally-verified, full extraction suite green (9 files, 79 assertions, 0 failures). v0.6.17 merged (PR #162, `8c56070a`, 2026-07-10); a post-merge Fable review the same day found these 6 correctness bugs in the shipped code (see "Post-Merge Regression Findings" below), all closed on this branch. **Note:** the unrelated `kg-config-silent-overwrite` fix (`ac70b490`) was merged onto this same branch (`v0.6.18-fix-extraction-regressions`) per an explicit user decision on 2026-07-10 to ship both as one combined release rather than two — different root cause/subsystem, same branch and version number now. See `knowledge/issues/kg-config-silent-overwrite/`.
