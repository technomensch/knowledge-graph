...
2. **Default to current-project scoping.** Change the default behavior so extraction filters
   to the active KG's project name (derivable the same way `kg-config.json`'s `active` /
   `graphs.<active>.path` is resolved elsewhere in this codebase) unless the user explicitly
   opts into an all-projects run (e.g. `--all-projects`). This flips the safety default: opt-in
   for the broader, riskier case rather than opt-in for the safe case.
