...
3. **Capture-pipeline command cluster** — an inventory found **14** commands/skills/agents doing overlapping-sounding work: commands `kmg-capture-lesson`, `kmg-update-graph` *(retired 2026-09-04, replaced by `kmg-backfill` — see ADR-071)*, `kmg-update-issue-plan`, `kmg-sync-all` *(retired 2026-09-04 — see ADR-071)*, `kmg-link-issue`, `kmg-meta-issue`, `kmg-start-issue-tracking`, `kmg-session-summary`, `kmg-handoff`; skills `kmg-lesson-capture`, `kmg-session-wrap`, `kmg-capture-router`; agents `knowledge-reviewer` *(retired 2026-09-04 — see ADR-071)*, `session-documenter`. Their names do not signal which pipeline stage each belongs to, producing "which command do I run" friction (confirmed via direct maintainer usage, not just theory). Mapped but not resolved — now specced as **[[ENH-034]]**.
...
| **T6** | Whether to rename `kmg-update-graph` to something like `kmg-ingest-graph` | [[ENH-034]] | 2 — wait for T9 | ✅ Decided 2026-09-04 — removed instead (Option C), not renamed. See ADR-071. |
| **T7** | Whether to rename `kmg-update-issue-plan` to something like `kmg-propagate-issue-plan` | [[ENH-034]] | 2 — wait for T9 | ✅ Decided 2026-09-04 — kept as-is (Option A), no rename |
...
- **Tier 2 — RESOLVED 2026-09-04:** T6 (rename `kmg-update-graph`?) → decided: removed instead (Option C, see ADR-071), not renamed. T7 (rename `kmg-update-issue-plan`?) → decided: kept as-is (Option A), no rename.
