...
- [Fix Ownership Follows the Root-Cause KG, Not the Code's Location](patterns/Lessons_Learned_Patterns_Fix_Ownership_Follows_Root_Cause_KG_Not_Code_Location.md) — When a bug lives in shared plugin code but its root cause is scoped to one KG's conventions (personal vs. project), route tracking/fix ownership to that KG, not to wherever the code physically resides
