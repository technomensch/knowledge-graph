...
- **ADR-028** — platform-agnostic rules as source of truth (vocabulary: "platform-agnostic", "source of truth", "deployment", "two-level hierarchy")
...
2. Task B2 was missing entirely — the rule was not written to `knowledge/rules.md` (in-repo, deployed with the plugin to all LLMs and users), which ADR-028 requires
...
- Tags: recall, planning, query-strategy, two-query-pattern, ADR-028, ENH-016
