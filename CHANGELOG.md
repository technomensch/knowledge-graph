...
- **New `kg_upgrade` category `stale-fts5-index-format`** — offers an opt-in, non-destructive rebuild of a project-local search index still sitting at the pre-v0.7.4 name-only path (see the index-collision fix below). Fires only when an old-format file exists and no new-format one has been built yet, so it stops appearing as soon as it has been applied; never fires for the personal graph, which has always used a fixed singleton path. Applying it re-reads the graph's own markdown and writes a fresh index — nothing in the knowledge graph is modified, and the old index file is left in place to delete at your convenience. Requires no `confirmBackfix` (it touches no knowledge file, only the cache under `~/.kmgraph/index/`).
- **New `kg_upgrade` category `stale-handoff-packages-location`** — offers an opt-in migration of any `./handoff-packages/<date>/` folders left over from before issue-31's default-path fix (see below) into `knowledge/handoffs/<date>/`. Fires only when at least one such dated folder exists; never fires for the personal graph. Non-destructive: a file identical to one already at the destination is deduplicated, a file that genuinely differs is left untouched on both sides and reported for manual review, and a folder is only removed once it's fully empty. Requires no `confirmBackfix`.

### Changed

- **Project-local FTS5 index files moved from `~/.kmgraph/index/projects/<name>.db` to `~/.kmgraph/index/projects/<name>-<pathHash>.db`.** This is a user-visible location change affecting `kg_search`, `kg_fts5_status`, and `kg_fts5_rebuild`: an index built before v0.7.4 is no longer read, and searches fall back to a (correct, slower) linear scan until the index is rebuilt. Use the `stale-fts5-index-format` upgrade category above to migrate, or just run `kg_fts5_rebuild`. `kg_fts5_status` now reports the authoritative path for a graph — prefer asking it over reconstructing the filename by hand. The personal KG's path (`~/.kmgraph/index/personal.db`) is unchanged.
- **New `KG_INDEX_DIR` environment override** for the FTS5 index root, mirroring the existing `KG_CONFIG_PATH` precedent. Primarily a test-isolation mechanism — the Jest suite and `tests/test-mcp-tools.sh` now point it at a temp directory so runs stop writing into the real `~/.kmgraph/index/`.

### Fixed

- **`kg_search` could silently return another project's results — or none at all — when two knowledge graphs shared a `name`.** The project-local FTS5 index path was keyed on the graph's name alone and never on where the graph actually lived, so any file present at that name-keyed path was queried as if it belonged to the graph being searched, which also suppressed the correct linear-scan fallback for a never-indexed graph. The filename now includes a digest of the graph's real (`~`-expanded, symlink-resolved) path, so unrelated same-named graphs can no longer collide. `kg_fts5_status` was also reconstructing this path inline from an unexpanded `~/...` string, making its reported `db_path` both drift from what `kg_search`/`kg_fts5_rebuild` used and vary with the caller's working directory; it now shares one computation with them while keeping its read-only contract (it still creates no directories). The KG-name portion of the filename is additionally sanitized so a name containing path separators cannot escape `projects/`. Closes issue-55.
...
- **New `kg_upgrade` backfix category `plan-status-drift`** — repairs implementation plans in `knowledge/plans/` whose Safety Header `STATUS` field was frozen at "STOPPED" from creation and never updated, even after the plan was approved or fully implemented. Two reliability tiers: mirror drift between the canonical `knowledge/plans/` copy and its `~/.claude/plans/` mirror is auto-repaired one-direction, behind the shared consent gate below; inferring true status from branch-merge history is report-only and never auto-applied, regardless of how strong the evidence looks. Closes issue-49.
- **New `kg_upgrade` backfix category `diff-blank-reconstruction`** — repairs session-summary files left with a permanently blank "files changed" section by the `main`-hardcoding bug fixed below. Classifies each blank file as correctly-blank (default branch, not a gap), reconstructable from git history, or unresolvable (commit gone) — never fabricates a file list for the unresolvable case. Closes issue-47.
- **Retroactive repair tool for `kg_capture`'s filename/frontmatter corruption** (new `capture-corruption` backfix category) — cleans up files already written with a doubled filename/ADR-number prefix or a doubled `---` frontmatter block, both caused by the guard gaps fixed below. Closes issue-46.
- **Shared consent gate for destructive `kg_upgrade` backfixes** — every backfix category that rewrites existing files now goes through one `confirmBackfixCategory()` gate instead of a patchwork of ad-hoc or missing checks. `capture-corruption` previously applied unconditionally with no consent check at all; that gap, along with the newer categories above, is now closed.

### Changed

- **`kmg-create-adr` and `create-adr-agent` collapsed into one implementation.** The command used to independently reimplement ADR filename generation, frontmatter assembly, file write, index update, and commit — a second copy that had already drifted from the agent's own path (mismatched author/email nesting, `commit_short`, an `implements` field that was always null on the command side). The command now dispatches to the agent with no partial context payload, and a new regression script (`scripts/check-adr-command-dispatch.sh`) guards against the two reimplementations re-diverging. Closes issue-48.
- **Plan Safety Headers now track real lifecycle state** — `AWAITING APPROVAL` → `IN PROGRESS` → `COMPLETE`, written by `kmg-execute-plan` at the actual approval and completion transitions, replacing a single frozen "STOPPED" string every plan was born with and nothing ever rewrote afterward. Plan completion also now asks whether the fix needs a matching `kg_upgrade` category, closing the gap where a backfill-worthy bug fix could ship with no way for already-affected content to get repaired. Closes issue-49, issue-50.
- **`kg_upgrade`'s upgrade wizard can now reach every backfix category, not just the original 6.** `/kmgraph:kmg-init`'s routing logic inverted from a hardcoded allow-list to a deny-list, so `capture-corruption`, `config-location`, and any category added to `upgrade.ts` in the future are reachable through the wizard's normal "Apply all" and per-item flows instead of requiring an undocumented raw MCP tool call. Closes issue-51.

### Fixed

- **`git diff main...HEAD` no longer hardcodes `main` or goes silently blank pre-branch.** Session-summary, `kmg-docs-impact-scan`, and `kmg-update-issue-plan` now resolve the repo's actual default branch (local `main`, then `master`) and diff against its merge-base with HEAD, falling back to uncommitted/staged changes (which also catches new untracked files) when HEAD is still on the default branch. Previously, any repo whose default branch wasn't named `main` — or any session run before its first commit — silently reported "no files changed." Closes issue-47.
- **`kg_capture`'s frontmatter/filename guards had 4 real gaps**, closed in the same pass that hardened them against the corruption the repair tool above cleans up: a file already carrying doubled frontmatter got a second doubled block re-stacked instead of cleaned up; the guard could delete genuine prose if every line happened to start with a colon-prefixed word (e.g. "Note: deferred X."); an empty `---`/`---` pair leaked through unstripped, producing four fence lines in a row; and a caller sending an already-prefixed title reproduced the original doubled-filename bug outright, with no server-side check at all. `kg_capture` now rejects a pre-prefixed title with a validation error before writing — a real behavior change for any caller currently sending one. Closes issue-46.
- **Dependabot: `nanoid` and `dompurify` bumped to patched versions and bounded below their next major** (nanoid ≥3.3.18, <4.0.0; dompurify ≥3.4.13, <4.0.0) — both are docs-tooling-only (Docusaurus build) dependencies, not part of the shipped plugin runtime. `image-size`'s two open advisories have no patched release yet and are tracked, not fixed.
- **Stale `docs/plans/` references corrected to `knowledge/plans/` across the codebase**, closing two hooks that had been silently broken since the path moved: `plan-mirror.sh` never actually mirrored a plan write into the active project's KG, and `session-end-prompt.sh` never actually warned about unfinished mid-plan work at session end — both exited early against a directory that no longer exists in this repo. Also fixed along the way: `session-summary-agent` and `kmg-execute-plan` could report "no active plan found" with one genuinely in flight; `kmg-docs-impact-scan` was silently diffing filenames-only instead of full patches in two places; and a plan-completion ordering bug where the Safety Header could be rewritten to `COMPLETE` before the KG-capture follow-up question had actually been asked.
...
- **`handoff-file-tracing-gate.sh` still hard-blocked sessions inside a git worktree when the handoff package's own files (`handoff-packages/`, gitignored by design) were generated in a different checkout** — issue-43's worktree-aware `REPO_ROOT` is correct, but `git worktree add` never checks out gitignored content, so a package generated in the main checkout (or another worktree) structurally can't exist under a different worktree's own root, regardless of anchor correctness. Now derives a `PKG_ROOT` fallback from `STARTHERE_PATH` — the transcript's own absolute `Read` path for the file that was actually opened — instead of a fresh `git` call, which also covers cross-worktree reads for free. The fallback gates on whether the `REPO_ROOT`-anchored path was actually `Read` (not merely whether it exists on disk) — an independent code review caught that a file-existence gate false-blocks when two checkouts' `handoff-packages/<date>/` directories collide (routine, since both are date-derived) and a same-named decoy happens to exist at the wrong root. Regression tests added (gitignored-package fixture, a negative twin confirming the fallback doesn't mask a genuinely unopened file, and a decoy-file-on-disk case). Closes #217, issue-44.
