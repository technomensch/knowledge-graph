...
| **v0.7.2** | Three new retroactive `kg_upgrade` backfix categories — run them if you are affected: `plan-status-drift` (plans in `knowledge/plans/` whose Safety Header `STATUS` was frozen at "STOPPED"; mirror drift against the `~/.claude/plans/` copy is auto-repaired, inferring status from branch-merge history is report-only and never auto-applied), `diff-blank-reconstruction` (session summaries left with a permanently blank "files changed" section by the `main`-hardcoding bug fixed in the same release), and `capture-corruption` (files already written with a doubled filename/ADR-number prefix or a doubled `---` frontmatter block). Run `kg_upgrade` (or `/kmgraph:kmg-init`, option — Verify/upgrade) to apply; every backfix that rewrites existing files now passes through one shared `confirmBackfixCategory()` consent gate first, and the wizard can now reach every category instead of only the original 6. **⚠️ Behavior change:** `kg_capture` now rejects an already-prefixed title with a validation error before writing — update any caller that sends one. Everything else (`kmg-create-adr`/`create-adr-agent` dispatch collapse, plan Safety Header lifecycle states, default-branch detection for `git diff main...HEAD`, `kg_capture` frontmatter/filename guard gaps, dependency bumps, stale `docs/plans/` reference fixes) is automatic after plugin reload. |
| **v0.7.3** | No upgrade action required. Prompt-hardening pass on shipped instruction files in `commands/`, `skills/`, and `agents/` (bare rule-word absolutes rewritten as if-then statements, agent-mechanics and internal-file-name disclosure wording harmonized across five sibling skills, an explicit correction/redaction path added to the session-summary Accumulated Narrative zone) — instruction wording only, no runtime behavior change, automatic after plugin reload. See [ADR-069](knowledge/decisions/ADR-069-prompt-hardening-project-instruction-files.md). |
| **v0.7.4** | **⚠️ Upgrade action needed if either migration applies to you — neither happens automatically.** Two new opt-in `kg_upgrade` categories: `stale-fts5-index-format` rebuilds a project-local search index still sitting at the pre-v0.7.4 name-only path (project-local FTS5 index files moved from `~/.kmgraph/index/projects/<name>.db` to `~/.kmgraph/index/projects/<name>-<pathHash>.db` as part of the `kg_search` same-name index-collision fix; until it is rebuilt an old index is no longer read and `kg_search` falls back to a correct but slower linear scan — plain `kg_fts5_rebuild` does the same job). `stale-handoff-packages-location` migrates any leftover `./handoff-packages/<date>/` folders into `knowledge/handoffs/<date>/` following the `kmg-handoff` default-path fix. Run `kg_upgrade apply` with the category you want (or `/kmgraph:kmg-init`, option — Verify/upgrade). Both are non-destructive, fire only when there is something to migrate, and need no `confirmBackfix`; neither fires for the personal graph, whose index path (`~/.kmgraph/index/personal.db`) is unchanged. Ask `kg_fts5_status` for a graph's authoritative index path rather than reconstructing the filename by hand. Remaining fixes (dead `docs/issue-tracker.md` reference, `kmg-handoff` auto-invoking `session-summary-agent` when the day has no summary, `kmgraph:recall` → `/kmgraph:kmg-recall` in hook-injected instructions) are automatic after plugin reload. |
| **v0.7.4.2** | Upgrade action available, not required: new opt-in `kg_upgrade` category `connect-unregistered-graph` registers an existing unregistered folder in place (no re-scaffold, no template writes) — the way out if `kg_config_init` ever refused to scaffold over a folder you already had `decisions/`/`lessons-learned/` content in. Reuses an orphaned `.kmgraph-id` marker's existing `graphId` when present (preserves continuity); otherwise derives a unique name from the directory basename. Not yet reachable from the `/kmgraph:kmg-init` wizard — run `kg_upgrade` with `apply: ["connect-unregistered-graph"]` directly (MCP or CLI). Goes through the same registration hard-block/broad-ancestor-warning guard as `kg_config_init` and the CLI's `runInit`. Remaining fixes (scaffold template routing corrected to match the wizard exactly, a scaffold-then-refuse file leak closed on both the MCP-native and CLI init paths) are automatic after plugin reload. |
| **v0.7.5–v0.7.8** | No upgrade action required. See [CHANGELOG.md](CHANGELOG.md) for full per-version detail; v0.7.8 was never released under its own version number (rolled into v0.7.9's release, below). |
| **v0.7.9** | Upgrade action available, not required: if your `rules.md` has the `<!-- kmgraph-defaults -->` block from a version before ADR-037's content fix, `kg_upgrade` (or the new standalone `/kmgraph:kmg-upgrade` command) now offers an additive-only retrofit that appends the missing Knowledge Governance section into your existing block — never touches anything else you've written, and refuses (reports, doesn't guess) if your file has duplicate, orphaned, or malformed markers. Also new: a per-tool-call warning inside the MCP server itself if your session's process is still running older code than what's installed on disk (closes issue-32) — no action needed, just reload when told to. Everything else (standalone `/kmgraph:kmg-upgrade` command, Claude Code/Gemini CLI session-start upgrade nudges, `checkVersionMismatch` downgrade-wording fix, `lastAppliedVersion` auto-advance on a clean inspect) is automatic after plugin reload. |
